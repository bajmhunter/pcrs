 ...
 */
        if( $_GET['action'] == 'Update' )
        {
            ...
            $state = $_GET['state'];
            $custiD = $_GET['id'];
            $db->runQuery("update customers set first_name = '$fName',last_name = '$lName',email = '$eMail',
                    street = '$street',suite = '$suite',city = '$city',state = '$state',
                    zipcode = '$zipCode' where id = $custiD");
            if($db->result)
            {
                $_SESSION['msg'] = '<div class="message">Customer Profile Updated!</div>';
                $custID = $custiD;
            }

            else
            {
                    $_SESSION['msg'] = '<div class="message error">Customer Profile Update Failed!</div>';
            }
        }


        $db->runQuery("select id,first_name,last_name,email,street,suite,city,state,zipcode,type,status,last_access
...
$('#profile-h1').prepend('Customer ID: ',pdata['id']);

var str= ['<h3>Customer Profile Information</h3><table class="profile_sect">'];
